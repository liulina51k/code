...
function checkevent() {
	...
	$(".telephone").focus(function(){
		$(".testphone").attr("style","display:none");
	})
	...
	$(".verifycode").focus(function(){
		$(".testcode").attr("style","display:none");
	})
...
function checkphone() {
	...
	if(telephone == ''){
		$(".testphone").removeAttr("style");
		$(".testphone").html("<p style='font-size:12px; color:#cc0000;'>手机号不能为空!</p>");
		return false;
	}else{
		var pat = /^(13[0-9]|15[0-9]|17[0-9]|18[0-9])\d{8}$/;
		if(!(telephone.match(pat))) {
		   $(".testphone").removeAttr("style");
		   $(".testphone").html("<p style='font-size:12px; color:#cc0000;'>手机号输入不正确!</p>");
		   return false;
		}
		$.getJSON(iisssite+"/do.php?do=phone&inajax=1&ac=checkphone&phone="+telephone+"&jsoncallback=?",function(v){
			   if(v == 1){
			   	   $(".testphone").removeAttr("style");
			   	   $(".testphone").html("<p style='font-size:12px; color:#cc0000;'>手机号已经存在!</p>");
		   		   return false;
			   }else {
			   	   $(".testphone").html("<p style='font-size:12px; color:#A9E247;'>输入正确</p>");
			   	   check = 1;
			   }
	    });
	}
...
function checkcode() {
	...
	if(verifycode == ''){
		$(".testcode").removeAttr("style");
		$(".testcode").html("<p style='font-size:12px; color:#cc0000;'>验证码不能为空!</p>");
		check = 0;
		return false;
	}else{
		var pat = /^\d{6}$/;
		if(!(verifycode.match(pat))){
			 $(".testcode").removeAttr("style");
			 $(".testcode").html("<p style='font-size:12px; color:#cc0000;'>验证码6位，且为数字!</p>");
			 return false;
	    }
	    $.getJSON(iisssite+"/do.php?do=phone&inajax=1&ac=checkverifycode&phone="+telephone+"&code="+verifycode+"&jsoncallback=?",function(v){
		   if(v == 0)
		   {
		   	   $(".testcode").removeAttr("style");
		   	   $(".testcode").html("<p style='font-size:12px; color:#cc0000;'>验证码输入不正确!</p>");
			   return false;
		   }
		   else if(v == -1)
		   {
		   	   $(".testcode").removeAttr("style");
		   	   $(".testcode").html("<p style='font-size:12px; color:#cc0000;'>验证码已过期，请重新获取!</p>");
			   return false;
		   }else
		   {
		   	   $(".testcode").html("<p style='font-size:12px; color:#A9E247;'>输入正确</p>");
		   	   check = 1;
		   }
	    });
	}
...
function clock() {
	...
	{
		num -= 1;
		$(".huoquhou").html("获取验证码(<span id='sp1'>"+num+"</span>)");
	}else {
		$(".huoquhou").attr("style","display:none");
		$(".huoqu").removeAttr("style");
	};		
}
function clock1() {
	$(".huoqu").html("<a href='javascript:void(0)' onclick='getCode();'>获取验证码</a>");
}
function getCode() {
	if(check)
	    var telephone = $(".telephone").val();
	...
	$.getJSON(iisssite+"/do.php?do=phone&inajax=1&ac=getverifycode&phone="+telephone+"&jsoncallback=?",function(v){
		if(v == 1)
		{   
			$(".huoquhou").removeAttr("style");
			$(".huoqu").attr("style","display:none");
		}else{
			$(".huoqu").html("获取验证码失败!");
...
function update() {
	$("#TB_overlay").removeAttr("style");
	$("#tanchuang").removeAttr("style");
}
function close() {
